...
class ReplayEngine:
    ...
    def process_current_frame(self):
        ...
        if not outage_status["outage_active"] and not outage_status["is_blending"]:
            ...
            # Correct Anchored Speed Reconstruction:
            # Exp_5 model predicts delta_v from anchor speed fix: v_reconstructed = max(0, v_anchor + delta_v)
            dt_step = 0.1
            v_anchor = self.outage_controller.anchor_speed_mps
            v_reconstructed = max(0.0, v_anchor + pred_delta_v)
            
            # ZUPT standstill check
            if accel_var < 0.02 and v_reconstructed < 0.3:
                v_reconstructed = 0.0

            # Advance 2D kinematic position using AI velocity + Gyro yaw rate (bias-corrected)
            # In outage, heading advances via yaw_rate_raw integration with pre-outage estimated bias subtracted
            ...
            dr_state = self.nav_core.propagate_step(
                ...
                heading_deg=None,  # Use gyro integrated heading
                dt=dt_step,
                accel_var=accel_var
            )
